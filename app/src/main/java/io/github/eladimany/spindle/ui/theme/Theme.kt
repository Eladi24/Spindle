...
// Every role is specified explicitly — lightColorScheme()/darkColorScheme() default any
// role you omit to Material3's own baseline purple, not a tint derived from what you did
// pass, so a partial scheme ends up as a mismatched patchwork of custom + baseline colors.
...
fun SpindleTheme(
    ...
    // Off by default — Spindle now has its own fixed identity (see Color.kt) instead of
    // deriving from the device wallpaper. Still available to flip on for comparison.
